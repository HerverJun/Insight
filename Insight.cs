...
namespace Insight
{
    public partial class Insight : Form
    {
            ...
            try
            {
                ...
                switch (action)
                {
                    ...
                    case "export_dataset_zip":
                        ...
                        break;
                    case "start_kaggle_training":
                        this.BeginInvoke(() => HandleStartKaggleTraining(root.Clone()));
                        break;
                    case "download_kaggle_output":
                        this.BeginInvoke(() => HandleDownloadKaggleOutput(root.Clone()));
                        break;
                    case "open_output":
        ...
        private async void HandleStartKaggleTraining(JsonElement data)
        {
            try
            {
        ...
        private async void HandleDownloadKaggleOutput(JsonElement data)
        {
            try
            {
...
";
            ...
            try
            {
                ...
                await Task.Run(() =>
                {
                    ...
                    try
                    {
                        ...
                        exitCode = _trainingProcess.ExitCode;
                    }
                    catch (Exception ex)
                    {
                    ...
                    finally
                    {
                        ...
                        {
                            SendToFrontend(new { action = "training_stopped" });
                        }
                        else if (!started || exitCode != 0)
                        {
                            var codeText = exitCode.HasValue ? exitCode.Value.ToString() : "未启动";
                            SendError($"训练未成功完成，退出码: {codeText}。已跳过 ONNX 导出。");
                            SendToFrontend(new { action = "training_finished", success = false });
                        }
                        else
                        {
                            var bestPt = FindBestPtForCompletedRun(workDir, trainingStartTime);
                            if (string.IsNullOrEmpty(bestPt))
                            {
                                SendError("训练完成，但未找到本次训练产生的 best.pt。已跳过 ONNX 导出。");
                                SendToFrontend(new { action = "training_finished", success = false });
                            }
                            else
                            {
                                var exported = ExportOnnx(pythonPath, workDir, bestPt, paramsData);
                                SendToFrontend(new { action = "training_finished", success = exported });
                            }
        ...
        private string? FindBestPtForCompletedRun(string workDir, DateTime trainingStartTime)
        {
        ...
        private bool ExportOnnx(string pythonPath, string workDir, string bestPt, TrainingParams paramsData)
        {
        ...
        private async void HandleConvertModel(JsonElement data)
        {
            // 手动转换调用
            try
            {
