...
namespace Insight
{
    public class YoloDatasetExporter
    {
            ...
            Action<int>? onProgress)
        {
            ...
            onLog?.Invoke("开始生成 YOLO 数据集目录...", "info");
            ...
            var splitIndex = Math.Clamp((int)(shuffled.Count * splitRatio), 1, Math.Max(1, shuffled.Count - 1));
            ...
            GenerateDataYaml(targetPath, classes, normalizedYoloVersion);
            GenerateTrainingConfig(targetPath, normalizedYoloVersion, modelSize, epochs, batchSize, imgSize, patience, workers, gpuIndex);
            GenerateColabNotebook(targetPath);
